...
<head>
	...
	<link rel="stylesheet" href="style/iframe_embed.css">
</head>
...
function showVideostoreVideos(videos)
{
	...
	function createCarouselItems(){
		...
		for(var i=0; i<numOfSlides; i++){
			...
			for(var j=0; j<3; j++){
				if(videos[c]){
					if(videos[c].title.length > 50){
						var title = videos[c].title.substr(0, 50) + "...";
					}else{
						var title = videos[c].title
					}
					...
					items += '\
						<div class="col-sm-4">\
							<div class="col-sm-10 col-sm-push-1">\
								<!-- (1) video wrapper -->\
								<div class="youtube" data-embed="'+videos[c].youtubeid+'"> \
								    <!-- (2) the "play" button -->\
								    <div id="v"'+videos[c].videostorevideo_id+' class="play-button"></div>\
								</div>\
								<p>\
									<h4>'+title+'</h4><br>\
